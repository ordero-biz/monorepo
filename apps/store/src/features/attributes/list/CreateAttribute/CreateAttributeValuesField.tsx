...
import type { CreateAttributeValuesFieldProps } from './types';
import {
  getAttributeValueFieldId,
  getEmptyAttributeValueField,
} from './utils/fields';
import { validateAttributeValueStatus } from './utils/validations';

export const CreateAttributeValuesField = ({
  form,
  open,
}: CreateAttributeValuesFieldProps) => {
        ...
        <form.Field name="attributeValues" mode="array">
          {(field) => {
            const lastAttributeValue =
              field.state.value[field.state.value.length - 1]?.value ?? '';
